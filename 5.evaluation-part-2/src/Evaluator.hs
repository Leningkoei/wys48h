...
--   mapM unpacker parameters >>=
--   \ (x:y:rest) ->
--     let iterator :: Bool -> a -> [a] -> Bool
--         iterator accumulator _ [] = accumulator
--         iterator False _ _ = False
--         iterator (previous `op` current) current rest
--     in return . Bool $ iterator (x `op` y) y rest
...
-- or :: [LispVal] -> ThrowsError LispVal
-- or parameters = mapM unpackBool parameters >>=
--   return . Bool . foldl1 (||)
...
eval (List [Atom "if", checkForm, thenForm, elseForm]) = do
  result <- eval checkForm
  case result of
    Bool True -> eval thenForm
    Bool False -> eval elseForm
    _ -> throwError $ TypeMismatch "bool" checkForm
